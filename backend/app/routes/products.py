...
product_bp = Blueprint("products", __name__, url_prefix="/api/admin/products")
...
def index_all_products():
    ...
    products = (
        db.session.execute(db.select(Product).order_by(Product.product_id))
        .scalars()
        .all()
    )
...
def add_product():
    ...
    product_missing = [
        field for field in product_required_fields if not data.get(field, "").strip()
    ]

    variant_missing = [
        field for field in variant_required_fields if not data.get(field, "").strip()
    ]
    ...
    product = Product(
        ...
        url_tag=data.get("url_tag", "").strip() or None,
    ...
    try:
        ...
        abort(
            400,
            description="Price must be a valid decimal and stock quantity must be a valid integer",
        )
    ...
    variant = ProductVariant(
        ...
        external_product_id=data.get("external_product_id", "").strip() or None,
    )
    if variant is None:
    ...
    try:
        ...
        db.session.rollback()
        flash(
            "PLEASE TRY AGAIN YOU ARE DUPLICATING A VALUE INSIDE THE FORM WHICH ALREADY EXISTS",
            "error",
        )
...
def edit_product(product_id):
    ...
    return render_template(
        "products/editproduct.html", product=product, variants=product.variants
    )
...
def update_product(product_id):
    ...
    try:
        ...
        product.url_tag = data.get("url_tag", "").strip() or None
        product.additional_notes = data.get("additional_notes", "").strip() or None

        variant.is_active = "is_active" in data
        variant.color = data.get("color", "").strip() or None
        variant.size = data.get("size", "").strip() or None
        ...
        variant.external_source = data.get("external_source", "").strip() or None
        variant.external_product_id = (
            data.get("external_product_id", "").strip() or None
        )

        db.session.commit()
        return redirect(
            url_for("products.index_all_products", product_id=product.product_id)
        )
    except IntegrityError:  # handle dups
        db.session.rollback()
