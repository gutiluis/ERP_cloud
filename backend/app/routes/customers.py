...
customer_bp = Blueprint("customers", __name__, url_prefix="/api/admin/customers")
...
def index_all_customers():
    ...
    """
    customers = (
        db.session.execute(db.select(Customer).order_by(Customer.customer_id))
        .scalars()
        .all()
    )
...
def submit_customer_form():
    ...
    try:
        new_customer = Customer(
            ...
            customer_status=CustomerStatus(required_fields["Customer Status"]),
            # nullable=False in the model
            customer_type=CustomerType(request.form.get("customer_type", "business")),
        )
    ...
    # exception for repeated customer_id, customer_name, customer_email unique=True
        ...
        current_app.logger.error(
            f"[LOG] Customer has same customer_id, or same customer_name or same email: {error}"
        )
    ...
    # integrityrror is a sublass of sqlalchemyerror
        ...
        db.session.rollback()
        current_app.logger.exception("Database fail. Creation unsuccessfull.")
